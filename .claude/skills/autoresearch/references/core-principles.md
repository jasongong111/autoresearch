...
**Apply:** Commit before verify. Revert on failure. Agent reads its own git history every iteration (`git log --oneline -20`, `git diff HEAD~1`) to inform the next experiment. See `references/autonomous-loop-protocol.md` for the full Git Memory integration.
