...
def test_task_project_gemma4_trace_api_preserves_payloads(client: TestClient, tmp_path: Path):
    task_project = tmp_path / "tasks" / "gemma4-skill-optimization"
    task_project.mkdir(parents=True)
    (task_project / "autoresearch-results.tsv").write_text(
        (FIXTURES / "autoresearch-results.tsv").read_text()
    ...
    trace_dir = task_project / ".autoresearch"
    trace_dir.mkdir()
    (trace_dir / "gemma4-trace.jsonl").write_text((FIXTURES / "gemma4_trace_structured.jsonl").read_text())
    state = client.app.state.dashboard  # type: ignore[attr-defined]
    state.refresh_runs()
    ...
    assert len(gemma4_trace["events"]) == 2
    assert gemma4_trace["events"][0]["phase"] == "api_request"
    assert gemma4_trace["events"][0]["request"]["messages"][1]["content"] == "What is 40 + 2?"
    assert gemma4_trace["events"][1]["response"]["choices"][0]["message"]["content"] == "42"
